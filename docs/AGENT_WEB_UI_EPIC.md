...
1. **A second live surface.** The host layer (`host`) was deliberately built surface-agnostic
   so a web chat could reuse the whole thing and swap only stdin/stdout for a socket. `surfaces/chat`
...
- **Not a rewrite of the host.** No web framework or proto types leak into `host`, the same
...
`surfaces/web` is a new submodule (own go.mod) importing `host`: the goapplib shell, the Connect
...
`surfaces/chat` is thin over the host.
...
### E1 (1194) — Session event log on `gocurrent.Queue` (`host`) — SHIPPED
...
### E2 (1195) — Pending-ask barrier (`host`) — SHIPPED
`barrierElicit` (`host/ask.go`) wraps the local `ElicitationUI` the coordinator drives: it emits a
...
- **`App.Subscribe(ctx) <-chan HostEvent` (`host/subscribe.go`)** — the async subscriber adapter
...
- Accept (met): a Connect client `Watch`es a live session and `Submit`s a turn; a local Observer and a
  ...
  over the wire wins an ask the local UI is blocking on (`surfaces/web/host_bridge_test.go`, run with
...
switch). Saved-layout reconcile carries over. The frontend lives in `surfaces/web/web/`; the built esbuild
bundle is committed under `surfaces/web/static/` (Go embeds it, so `just test-agent` needs no Node step).
...
provider) + `surfaces/web/run.sh` as the runnable proof. Unit tests (vitest) cover Frame-decode/dispatch, the
