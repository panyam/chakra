...
**What this is.** A living assessment of the mcpkit `` host layer against (a) general agent
...
direct inventory of `` source + open-issue sweep. Verdicts respect `CONSTRAINTS.md`
...
constraint **A8** in `CONSTRAINTS.md`. The remaining phases stay open as epics, with **Phase 5
...
layer we deliberately scope to a coding agent built *on* ``, not `` itself (#1059). It is
...
command. Roughly tools 60%, prompts 25%, harness 15% — and the harness slice is what `` already
...
| **Eval / scorer harness** | 🟡 #974,#932 | `eval/` — `Case`/`Scorer`/`Suite`/`Scenario`; 8 deterministic scorers; `Judge` (build-tagged); LongMemEval *smoke* scenarios | external-benchmark adapter **⏳ #1015**; real LongMemEval loader **⏳ #1014** |
| **Persistence (RunStore) + fork/rewind** | ✅ #960,#962,#963,#986 | `runstore.go` — full interface + `InMemoryRunStore`; redis + gorm (pg/sqlite) backends; `ForkRun{AtMessage}` checkpoint fork; `ListRuns`; `Message.Timestamp` | retention/GC **⏳ #999** |
| **Per-tool-call cancellation / interrupt** | ✅ #936,#937 | `runner.go` `TurnRequest`/`Control` channel (per-`CallID`); `EventToolCancelled` | — |
| **Working memory** | ✅ #938,#1003,#1140 | `memory.go` — `MemorySource` remember/recall/forget; `Summary`/`RecallRelevant`; `InMemoryMemoryStore`; durable + per-request `Namespace` backends (redis/gorm, #1003); session-scoping by run id (#1140) | sub-agent memory model (injection over shared store) **#1151** |
| **Semantic recall (vector)** | ✅ #940,#1019 | `embedder.go` (`Embedder`, `OpenAIEmbedder`), `semantic_memory.go` (`InMemorySemanticStore`), gorm **pgvector** store; pre-turn recall auto-injection | standalone doc-RAG VectorStore **⏳ #1021**; reranker **⏳ #1020**; auto-distillation **⏳ #1022** |
| **Compaction / summarization** | 🟡 #939,#1011 | `compaction.go` — `SummarizingCompactor`, `TokenEstimator`/`CharTokenEstimator`, `EventCompaction`; pre-loop hook; budgeted summary injection | mid-turn compaction **⏳ #1006**; real tokenizer **⏳ #1007** |
| **Tool-result offloading (context mgmt)** | ✅ #966,#971,#972 | `offloading_source.go` + `ToolResultStore` (mem/redis/gorm); `read_tool_result` (offset/limit/grep + two-sided context, bounded) | streaming/handle-based very-large results **⏳ #980,#979** |
| **Sub-agents (agent-as-tool)** | ✅ #941,#942,#943,#1031,#1032,#1033,#1035,#1036,#1042 | `agent_source.go` (`AgentSource`, depth+budget caps, structured I/O), `async_agent_source.go` (`AsyncAgentSource` Task form, #1035), `fanout_source.go` (`FanOutSource`, #1033), `team.go` (`Team` handoff + host wiring + tagging, #1042), `tree_budget.go` (`TreeBudget` aggregate cap, #1032), `signal.go` + `agent_pool.go` (upward signals + runner-control pool + interruptible turn, #1036), `SubAgentEvent` nesting, declarative host personas | dynamic catalog #1038, nested config #1043, interaction mediator #1157, map-style fan-out (unfiled) |
...
| **Coding surface (sandboxing, hooks, repo map, LSP)** | 🟡 **decided** — #1059 closed | Hooks landed in `` as `ToolMiddleware` (#1248). The rest are an **extension** over the general harness, not a separate module: epic **#1252**. First child shipped: file checkpoint / rewind **✅ #1267** in `ext/checkpoint`, the first real `host.Extension` — building it found the contract had no lifecycle hook, so `TurnStart` was added. Anchored edit primitive **✅ #1275** in `ext/files`: content anchors locate a hunk, a content hash detects the file changing underneath, and neither substitutes for the other. It landed as `files` rather than `coding` because nothing in it is code-aware, which splits the epic's children along a line worth keeping. Toolset completed **✅ #1284** — `write_file` (no create-or-overwrite mode; `expect_hash` alone separates create from replace), plus `list_files` / `search_files`, so an agent can find what it edits | LSP-in-loop **✅ #1301** landed as `ext/lsp`, named after the mechanism rather than the use case, on the same reasoning that made #1275 `files` rather than `coding`. It is the first `ContextStages` consumer and the first extension to own a subprocess, so it added `host.Extension.Close`. Building it found that **`ContextStage` is per-turn while an edit-check-fix loop is per-step**, which is why diagnostics ship on two paths; that plus three smaller seam gaps are recorded on #1240. Repo map **✅ #1311** could not be its own module: C4 blocks `ext/repomap` importing `ext/lsp`, and working out why showed the two would run two language servers over the same tree, so the map is a second contribution from `ext/lsp` gated by `Config.RepoMap`. Command execution and its sandbox **✅ #1312** landed as `ext/exec`, reordered ahead of the test loop because the epic shipped the dangerous half first. It also corrected the epic twice: sandboxing cannot live in a `ToolSource` wrapper (most tools are in-process functions or MCP calls with no subprocess to confine), and the sandbox is not the primary defense, the approval gate is. Only the darwin `sandbox-exec` backend ships, with Linux carved out to #1320. Remaining #1252 child: test/build loop (#1313), now unblocked. Still **blocked**: diff-level review (#1288). Host decomposition **#1251** runs in parallel |
...
``** (a lean, embeddable, extension-first harness with the same four surfaces), while oh-my-pi is
...
line sharply: **`` ships no tool implementations at all** (it is the SDK substrate), so most of
...
| oh-my-pi capability | Nature | mcpkit `` status |
|---|---|---|
...
| ~55k-LoC in-process Rust core (ripgrep/glob/shell/PTY/AST, no fork-exec) | coding-surface / perf | out of scope → **#1059**; `` is provider+loop, transport-agnostic by design |
...
coding agent on top of `` is the thing that would actually compete — the SDK is necessary but not
sufficient.
...
**Shape for the eval harness:** the internal `eval` harness (StubProvider + spans, ✅ shipped) is
the CI gate; the **external-benchmark adapter (#1015)** — starting with BFCL — is the missing second
layer. mcpkit's own `eval` scorer harness is itself a differentiator: only Genkit matches it in
the Go field.
...
| Area | Files |
|---|---|
| Loop / approval / cancel / structured | `runner.go`, `approval.go` |
| Providers | `provider.go`, `anthropic_provider.go`, `openai_provider.go`, `failover.go` |
| Memory | `memory.go`, `semantic_memory.go`, `embedder.go`, `compaction.go` |
| Persistence / offloading | `runstore.go`, `toolresultstore.go`, `offloading_source.go`, `store/{redis,gorm}` |
| Multi-agent | `agent_source.go`, `team.go` |
| Eval | `eval/` (`eval.go`, `scorer.go`, `judge.go`, `suite.go`, `scenario.go`, `longmemeval/`) |
| Tool layer / events / policies | `toolsource.go`, `multi_source.go`, `filter_source.go`, `events.go`, `injection.go`, `triggers.go`, `stages.go` |
| Host surface | `host/` (`commands.go`, `connections.go`, `render.go`, `hostevent.go`, `memory.go`, `subagents.go`, `persistence.go`), `surfaces/chat/` |
| Invariants | `docs/AGENT_DESIGN.md`, `CONSTRAINTS.md` |
