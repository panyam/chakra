...
- **Bridge → `` + `host`** (issue 1144): consuming a resolved
  ...
  - The host (`host/server_agents.go`) discovers each connected server's
...
- **A6** (model-facing → ``): `AgentSource`, `Team`, and the future
  control meta-tools are all model-facing, so they live in ``.
- **A2** (wire-serializable events): nesting rides the `SubAgentEvent`
  envelope (scope/depth on the wrapper); `Event` stays flat.
- **A1** (dependency direction): composition is pure `` over `Runner` +
...
- Control: the 1036 epic — upward signals (1165, A, PR 1168), runner-control
  ...
  **two-key** (`signal.go`): `dispatchSinkKey` (a dispatch's sink for its
...
- Surface: declarative `host` + agentchat multi-agent config and nested
